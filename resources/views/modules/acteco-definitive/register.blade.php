  ...
  -->--}}
                            ...
                            @foreach($company->ciu as $ciu)
                                ...
                                <div class="ciu-company">
                                    ...
                                    <div class="input-field col s12 m2 tooltipped" data-position="bottom" data-tooltip="Código del Clasificador Industrial Internacional Uniforme, el mismo facilita a los contribuyentes la ubicación de las actividades por sectores y algunos subgrupos con referencias específicas de su actividad económica (Ord.  AE I Parte Normativa, 6. Régimen Tarifario). .">
                        ...
                        <a href="#declaracion" class="btn btn-large btn-rounded waves-effect waves-light peach modal-trigger">Declarar
                          ...
                          <!-- Modal Structure -->
                                ...
                                <div class="modal-content left-align">
                                    ...
                                    <p>Yo, <b>{{ Auth::user()->name . " " . Auth::user()->surname }}</b> titular de la C.I. N° <b>{{ Auth::user()->ci }}</b> y de domicilio en <b>{{ Auth::user()->address }}</b>, declaro bajo Fé de Juramento que todos los datos y cifras que aparecen en esta declaración son una copia fiel y exacta de los datos contenidos en los libros de contabilidad y que han sido llevados de acuerdos con los principios de contabilidad  generalmente aceptados, igualmente autorizo suficientemente al <b>Servicio Municipal de Administración Tributaria (SEMAT)</b> a efecto de verificar la veracidad de lo aquí declarado.<br>A los <b>{{\Carbon\Carbon::now()->format('d')}}</b> días del mes <b>{{\Carbon\Carbon::now()->format('m')}}</b> del año <b>{{\Carbon\Carbon::now()->format('Y')}}</b>.</p>
                                </div>
