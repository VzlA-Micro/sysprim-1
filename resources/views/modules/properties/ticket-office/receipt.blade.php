    ...
    <table style="width: 100%; border-collapse: collapse;">
        <tr style="text-align: center">
            <td style="width: 25%;" rowspan="2">
                <img src="{{ url('images/alcaldia_logo.png') }}" style="width:180px; height:80px" alt="Image" width="100%" height="100%"><br>
                <span></span><br>
...
@if($taxes[0]->status==='verified'||$taxes[0]->status==='verified-sysprim' ||$taxes[0]->status==='exempt')
    ...
    @foreach($taxes as $taxe)
        ...
        @if($taxe->properties[0]->pivot->recharge != 0)
            <tr>
                ...
                <td style="width: 10%;font-size: 10px !important;"></td>
                <td style="width: 10%;font-size: 10px !important;">{{ number_format($totalAmount,2,',','.') }}</td>
                <td style="width: 10%;font-size: 10px !important;">{{ number_format($taxe->properties[0]->pivot->recharge,2,',','.') }}</td>
                @php $totalAmount += $taxe->properties[0]->pivot->recharge; @endphp
                <td style="width: 10%;font-size: 10px !important;">{{ number_format($totalAmount,2,',','.') }}</td>
            </tr>
        @endif
        ...
        @endif
        @if($taxe->properties[0]->pivot->discount != 0)
            <tr>
                <td style="width: 10%;font-size: 10px !important;"></td>
                ...
                <td style="width: 10%;font-size: 10px !important;"></td>
                <td style="width: 10%;font-size: 10px !important;">{{ number_format($totalAmount,2,',','.') }}</td>
                <td style="width: 10%;font-size: 10px !important;">-{{ number_format($taxe->properties[0]->pivot->discount,2,',','.') }}</td>
                @php $totalAmount -= $taxe->properties[0]->pivot->discount; @endphp
                <td style="width: 10%;font-size: 10px !important;">{{ number_format($totalAmount,2,',','.') }}</td>
            </tr>
        @endif
        @if($taxe->properties[0]->pivot->fiscal_credit != 0)
            <tr>
                <td style="width: 10%;font-size: 10px !important;"></td>
                ...
                <td style="width: 10%;font-size: 10px !important;"></td>
                <td style="width: 10%;font-size: 10px !important;">{{ number_format($totalAmount,2,',','.') }}</td>
                <td style="width: 10%;font-size: 10px !important;">-{{ number_format($taxe->properties[0]->pivot->fiscal_credit,2,',','.') }}</td>
                @php $totalAmount -= $taxe->properties[0]->pivot->fiscal_credit; @endphp
                <td style="width: 10%;font-size: 10px !important;">{{ number_format($totalAmount,2,',','.') }}</td>
            </tr>
        @endif
        <tr>
            ...
            <tr>
                <td style="width: 80%;">
                    <img src="data:image/png;base64, {!! base64_encode(QrCode::format('png')->errorCorrection('H')->merge('/public/images/pdf/sysprim.png', .18)->size(190)->generate(
                      ...
                      "MONTO:".number_format($totalAcum,2)."\n".
